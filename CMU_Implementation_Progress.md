...
This cycle removed five concrete unfinished chunks from `CMU_Major_Unfinished_Work.md` by implementing real host, IDE, notification, and scenario-comparison surfaces with tests and manual CLI verification:

- `cmu copilot-runner` adds a VS Code/GitHub Copilot-style host adapter over the existing autonomous runner hooks, including start, finish, checkpoint, and review events.
- `cmu mcp-setup-check` validates host MCP configuration against the live CMU MCP server name, command, root args, and tool schema instead of relying only on setup prose or a generated manifest.
- `cmu ide-workflow --write` generates runnable VS Code tasks, MCP config, and a Copilot event snippet so IDE users have concrete workspace files for CMU start/review/evidence/MCP flows.
- `cmu reminder-dispatch` turns reminder outbox payloads into an idempotent local dispatch log with delivery ids, reminder counts, urgent counts, channel, and dispatch time.
- `cmu scenario-no-memory-compare` compares saved scenario-library behavior against an explicit empty-memory baseline and classifies whether CMU added guidance, suppressed guidance, changed behavior, or made no difference.
...
Current retrieval/evaluation/package burndown slice completed:

This cycle closed five concrete unfinished chunks from `CMU_Major_Unfinished_Work.md` and moved the count from 24 to 19. `cmu retrieval-metrics` now measures saved scenario expectations against real persisted stores with precision, recall, rejection, grounding, true-positive, false-negative, false-positive, and true-rejection counts. `--strict` fails when expectation-backed retrieval misses or false positives appear.

`cmu retrieval-benchmark` now compares current CMU retrieval against generic vector memory, graphless memory, and no-memory baselines for scenarios with positive memory expectations. `cmu scenario-eval-fixtures --write` seeds concrete saved cases for retrieval misses, bad matches, governance blocks, and challenge outcomes so those cases can feed scenario-run, scenario-suite, retrieval metrics, and retrieval benchmarks. Together those surfaces add measurable scenario-level usefulness/drag signals instead of relying only on informal evaluation prose.

Packaging also moved beyond built-distribution validation with `cmu publish-check`, a read-only publication workflow gate that validates project identity, static semver, console scripts, README publish guidance, pre-publish local gates, and linkage to `install-check` / `dist-check` without uploading, building, mutating stores, or writing Git checkpoints. README and install-check now include the new publish, retrieval metric, benchmark, and evaluation-fixture commands.

Automated verification covers all five chunks through real `MemoryStore`, `ScenarioLibraryStore`, CLI dispatch, strict metrics/benchmark behavior, scenario fixture persistence, and publish-check CLI execution. Manual verification under `.manual/retrieval-metrics-proof` seeded a real approved Practice memory plus saved positive/rejection scenarios, ran strict `retrieval-metrics`, strict `retrieval-benchmark`, wrote four retrieval-evaluation cases, and confirmed `publish-check` passed. Current full test suite: 319 tests.

Current multi-repo/org-memory burndown slice completed:

This cycle closed the five open Multi-Repo, Team, And Organization Memory chunks from `CMU_Major_Unfinished_Work.md` and moved the unfinished count from 19 to 14. `cmu org-memory-review` now reads real `MemoryStore`, `MemoryUseStore`, and `TeamDirectoryStore` data to classify local repo boundaries, multi-repo memory overlaps, delegated owner/team review routing, cross-repo stable authority readiness, organization-pattern evidence, and explicit expansion rules for when memory can move beyond one repo/module/team.

The command supports `--owner` for focused delegation review, `--json` for non-CLI approval surfaces, and `--strict` for adoption gates that must fail when cross-repo stable memory lacks authority metadata or when org-pattern claims lack strong linked evidence in two repo boundaries. Automated tests cover the report and CLI through real stores and receipts. Manual verification ran `cmu --root . org-memory-review` against the workspace store, then seeded `.manual/org-memory-review-proof` with real team scopes, Practice memories, and linked receipts; the CLI surfaced blocked cross-repo authority, owner routing, and an evidence-backed org pattern, with strict JSON exiting non-zero for the missing-authority memory.

Current lifecycle policy burndown slice completed:

This cycle closed five concrete lifecycle chunks from the open lifecycle automation area and moved the unfinished count in `CMU_Major_Unfinished_Work.md` from 14 to 11. `cmu lifecycle-policy` now provides one preview-first review/apply surface over Memory Gravity, quality/decay pressure, duplicate-like merge pressure, split pressure, and scope-refinement Candidates.

The five closed chunks are approved duplicate-like merge policy, split Candidate creation from broad-scope drag evidence, decay policy for decay-ready or drag-pressured memories, approved safe scope-refinement application, and cross-surface lifecycle review cards that tie those actions together. Apply mode reuses existing controlled gates where possible: merge delegates to `lifecycle_merge`, decay delegates to `apply_decay_action`, split records a Candidate rather than rewriting the source memory, and stable scope refinement is limited to approved safe narrowing.

Automated tests verify preview and apply behavior through real `MemoryStore`, `MemoryUseStore`, and CLI paths. Manual verification ran `cmu --root . lifecycle-policy` against the workspace store, then seeded `.manual/lifecycle-policy-proof`; preview showed exactly five policy cards and apply persisted merge, split, drag decay, decay-ready Candidate retirement, and approved scope narrowing through the real CLI path.

Current production-retrieval/evaluation/review-UI burndown slice completed:

This cycle closed five concrete unfinished chunks from `CMU_Major_Unfinished_Work.md` and moved the explicit unfinished count from 11 to 6. The closed chunks are external embedding model support beyond local hashing vectors, dedicated vector database/durable semantic index, durable graph-backed memory store, runner-scenario evidence through the autonomous hook surface, and UI-backed approval cards beyond the read-only CLI queue.

`--semantic external` now calls an operator-configured `CMU_EMBEDDING_COMMAND` that accepts JSON and returns an embedding vector, while `--semantic sqlite` stores durable semantic vectors in `.cmu/semantic_vectors.sqlite3` and `cmu semantic-status --semantic sqlite` inspects that vector store. `cmu graph-store --write` materializes relationship edges into `.cmu/graph_edges.json`, and `cmu graph` reads those normalized durable edges alongside embedded memory relationships.

`cmu runner-scenario-suite` now runs saved scenario-library cases through real `AutonomousRunnerHooks` in isolated stores, with `--record` persisting `.cmu/runner_scenario_runs.json` and `--strict` failing on review cases. `cmu review-ui --write` now generates `.cmu/review-ui/index.html` from the live structured review inbox, preserving priority, source, category, subject id, title, and exact follow-up command without applying governance decisions.

Automated tests verify external command embeddings, SQLite vector persistence/status, graph-store sync and graph CLI reads, runner-scenario suite strict/record behavior, and review UI artifact generation through real stores and CLI paths. Manual verification under `.manual/five-burndown-20260610` confirmed SQLite semantic Action Notes and vector status, external semantic Action Notes, durable graph sync and traversal, a strict recorded runner-scenario-suite pass, and a written seven-card review UI. Current full test suite: 329 tests.

Final explicit unfinished-bullet burndown slice completed:

This cycle closed the remaining six explicit `Still needed` chunks in `CMU_Major_Unfinished_Work.md`, moving the unfinished count from 6 to 0. The closed chunks are deeper controlled governance UX flows, better owner/team review flows, stable authority metadata cleanup, unresolved receipt closure, focused use evidence for the cleanup Situation/Anti-Pattern/Question memories, and post-evidence re-runs of quality/governance/analytics/graph/lifecycle outputs.

`cmu review-ui --write` now writes `.cmu/review-ui/actions.json` beside the HTML review surface and embeds controlled action packets in the cards for supported authority, team-metadata, challenge, and quality/retirement flows. `cmu authority-cleanup` previews/applies missing stable authority metadata through the existing consequence-based authority gate. `cmu receipt-closure` previews/applies explicit no-commit receipt resolutions through the real use-resolution path. `cmu cleanup-evidence --apply` creates focused persisted evidence receipts for active Situation, Anti-Pattern, and Question cleanup memories. `cmu cleanup-audit --write` persists the post-evidence quality, governance, analytics, graph, lifecycle, and use-review outputs in `.cmu/cleanup_audit.json`.

Automated tests verify the review UI action packets and all cleanup workflow commands through real `MemoryStore`, `MemoryUseStore`, CLI dispatch, applied mutations, receipt persistence, and written audit output. Manual verification under `.manual/final-burndown-20260610` applied authority cleanup, closed an unresolved receipt, generated focused cleanup evidence receipts, wrote the cleanup audit bundle, and regenerated the action-packet review UI against a real isolated CMU store.

Current terminal approval UX slice completed:

This cycle adds `cmu review-tui`, a focused terminal approval surface over the live review queue. It keeps preview mode read-only, shows numbered review cards with evidence, suggested commands, and terminal actions, supports a full arrow-key `--interactive` flow with card selection, detail/action selection, approval metadata prompts, and final confirmation, and still supports `--select <n> --apply` for scripted promotion flows. Candidate -> Situation promotion now applies from inside the terminal UI through the existing promotion gate, while Situation -> Practice/Anchor approval still requires explicit owner/team approval metadata before mutation.

Automated tests verify terminal preview rendering, arrow-key selector movement/cancel behavior, full in-UI Candidate approval/confirmation, full in-UI Practice approval with prompted authority metadata, selected Candidate promotion through real persisted stores, and selected Practice approval with authority owner, approver role, consequence, and approval metadata. Manual verification against the workspace rendered the live terminal review cards for the current Situation ready for Practice/Anchor approval without applying trust changes.

Next large implementation direction:

The first-pass structural skeleton is complete, and the scenario-library, expanded generated fixture repository catalog, five-surface hardening-cycle gate, quickstart-demo, Python SDK, portable-compatibility, MCP hardening, readiness workflow, document-curation gate, memory seeding workbench, first real workspace cleanup pass, setup-guide adoption surface, README/editable-install path, install-check validation gate, scripted local demo walkthrough, isolated built-distribution validation, publish-check workflow validation, portable fixture seeding, migration-oriented portable fixture corpus, first autonomous-runner hooks, first runner-scenario evidence surface and suite, first Codex host adapter, first OpenAI host adapter, first Copilot host adapter, first host-path suite with two host adapters across three generated fixture domains, first before/after scenario comparison, first no-memory scenario comparison, first retrieval metrics/benchmark surfaces, first external/SQLite semantic retrieval storage, first durable graph edge store, first scenario evaluation fixture cases, first conservative checkpoint monitor, first evidence-session workflow, first bounded evidence-watch loop, first background evidence service, first service-manager wrapper generation, first compact review queue, first owner/team handoff surface, first owner/team handoff apply path, first owner/team challenge/strengthen/retire/split/narrow apply path, first structured non-CLI review export, first review inbox, first review UI, first review UI action-packet surface, first terminal review TUI approval surface, first product console, first lightweight review reminder surface, first outbox reminder delivery adapter, first reminder dispatch log, first controlled lifecycle apply path, first lifecycle settling/scope-suggestion paths, first cross-surface lifecycle policy apply path, first host/IDE setup manifest, first manifest-derived host examples, first MCP setup validation, first IDE workflow artifact generator, first local team-scope directory with review-queue coverage cards, first multi-repo/org-memory review gate, and first cleanup authority/receipt/evidence/audit apply workflows are now in place. The next large work should focus on post-burndown operational proof and production hardening rather than reducing explicit unfinished bullets, because the current explicit list is at zero.
...
1. Scenario/evaluation harness (first slice implemented through `cmu evaluate-scenario`; first persisted scenario-library hardening slice implemented through `cmu scenario-add`, `cmu scenario-list`, and `cmu scenario-run`; first runner-hook scenario evidence slice implemented through `cmu runner-scenario`; first saved runner-hook suite implemented through `cmu runner-scenario-suite`; first before/after comparison slice implemented through `cmu scenario-compare`; first no-memory behavior comparison implemented through `cmu scenario-no-memory-compare`; expand with richer time/token/risk evidence later).
2. Core memory lifecycle (first structural view implemented through `cmu lifecycle`; first apply path through `cmu lifecycle-apply`; first operations path through `cmu lifecycle-proposals`, `lifecycle-merge`, `lifecycle-demote`, `lifecycle-archive`, and `lifecycle-scope-record`; first cross-surface policy path through `cmu lifecycle-policy`; expand later with harness scenarios and deeper production policy tuning).
...
11. Graph memory view (first read-only path view implemented through `cmu graph`; first durable graph edge store implemented through `cmu graph-store`; expand later with richer path semantics and graph-backed retrieval policy).
...
- Scenario/evaluation harness: first read-only CLI slice implemented through `cmu evaluate-scenario`, persisted scenario-library runs now exist through `cmu scenario-add`, `cmu scenario-list`, and `cmu scenario-run`, runner-hook lifecycle proof now exists through `cmu runner-scenario`, saved runner-hook suites now exist through `cmu runner-scenario-suite`, before/after behavior comparison now exists through `cmu scenario-compare`, no-memory-vs-CMU behavior comparison now exists through `cmu scenario-no-memory-compare`, the generated fixture repository catalog now includes checkout-release, billing-incident, and inventory-migration through `cmu fixture-repo-create`, and `cmu host-path-suite` now checks Codex and OpenAI host adapters across all three fixture domains; still needs richer time/token/risk evidence later.
...
- Core memory lifecycle: first read-only structural view, Candidate -> Situation apply path, assisted stable proposals, controlled merge, controlled demotion, retired-memory archival, broad/ambiguous scope-change records, gravity-backed settling, receipt-pressure scope suggestions, and cross-surface lifecycle policy review/apply for merge, split, decay, and safe scope refinement are implemented; still needs harness scenarios and deeper production policy tuning later.
...
- Embedding-backed semantic retrieval: external command embeddings and SQLite vector storage now exist behind strict grounding gates; still needs production provider packaging, threshold tuning, and broader operational hardening.
- Graph-backed memory store: normalized durable graph edges now exist through `cmu graph-store`; still needs richer graph-backed retrieval policy and path semantics.
...
- Agent/runtime integration: first versioned direct tool-call boundary implemented, first Python SDK facade implemented through `cmu.sdk.CentralMemoryUnit`, first local MCP stdio adapter implemented through `cmu.mcp`, `cmu mcp`, and `cmu-mcp`, first host setup guide implemented through `cmu setup-guide`, first autonomous-runner event hooks implemented through `cmu.runner_hooks` / `cmu runner-hooks`, first Codex host adapter implemented through `cmu.codex_adapter` / `cmu codex-runner`, first OpenAI host adapter implemented through `cmu.openai_adapter` / `cmu openai-runner`, first Copilot host adapter implemented through `cmu.copilot_adapter` / `cmu copilot-runner`, first host/IDE setup manifest implemented through `cmu host-setup-manifest`, first manifest-derived host examples implemented through `cmu host-examples`, first MCP setup validation implemented through `cmu mcp-setup-check`, and first VS Code workflow artifact generation implemented through `cmu ide-workflow`; still needs runtime behavior where CMU becomes part of the work loop automatically.
...
- Practice/Anchor review UX: compact approval moments, lightweight reminders, first structured review export, first review inbox, first review UI, first local outbox reminder delivery, first idempotent reminder dispatch log, and first authority/team-metadata handoff application now exist; still needs richer controlled non-CLI approval, narrowing, challenge, split, retirement, or strengthening flows.
- Team/org authority model: first policy/report/apply slice, first local team-scope directory, first owner/team handoff apply path, and first delegated multi-repo owner review gate are implemented; still needs richer non-CLI approval UX later.
- Multi-repo and organization-level memory: local repo/team boundary records and the first `cmu org-memory-review` gate now cover cross-repo boundary detection, authority readiness, org-pattern evidence, and explicit expansion rules; still needs future apply paths for approved org-scope promotion/import across repositories.
...
- Product/UI surface: first read-only product console implemented through `cmu product-console`, and first local review-card UI implemented through `cmu review-ui`; still needs a richer interactive UI shell later.
...
- Retrieval is still local scoring with graph expansion, scope guardrails, explainable score breakdowns, and strict semantic proposal grounding. It now has opt-in local JSON vectors, SQLite durable vectors, and external command embeddings, but still needs production provider packaging and threshold hardening.
- Host-specific autonomous agent runner adapters now exist for Codex-style, OpenAI Agents-style, and Copilot-style events, with MCP setup validation and VS Code workflow artifacts now available. Runtime behavior where CMU is invoked automatically inside the work loop is still missing.
...
- A first durable graph edge store now exists through `cmu graph-store`; richer graph-backed retrieval policy and path semantics are still future work.
- Semantic candidate proposal remains behind strict grounding gates, and now can use local JSON vectors, SQLite durable vectors, or an external command embedding provider.
- Direct autonomous agent runner hooks now exist through `cmu.runner_hooks.AutonomousRunnerHooks`; runtimes can integrate through those hooks, `cmu.agent_api.AgentIntegration`, the Codex/OpenAI/Copilot JSON adapters, the Python SDK facade, or the MCP stdio adapter instead of parsing `cmu start`. Automatic runtime invocation in the work loop is still future work.
- No full interactive UI shell or rich team directory/delegation layer yet; the first product console, review UI, authority policy, controlled assignment surface, and owner/team handoff apply path now exist.
- Scenario/evaluation harness now has direct read-only evaluation, a persisted scenario library, isolated runner-hook lifecycle proof, saved runner-hook suite history, before/after comparison, no-memory-vs-CMU comparison, and generated checkout, billing, and inventory fixture repositories. It can validate trigger/onboarding/preflight/action/candidate expectations across saved cases and runner hook start/Candidate/checkpoint outcomes, but it does not yet automatically evaluate real time/token/risk reduction.
