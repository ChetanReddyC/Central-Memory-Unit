...
The first-pass structural skeleton is complete, and the scenario-library, quickstart-demo, Python SDK, portable-compatibility, MCP hardening, readiness workflow, document-curation gate, memory seeding workbench, first real workspace cleanup pass, setup-guide adoption surface, README/editable-install path, install-check validation gate, scripted local demo walkthrough, isolated built-distribution validation, first autonomous-runner hooks, first runner-scenario evidence surface, first Codex host adapter, first before/after scenario comparison, first conservative checkpoint monitor, first compact review queue, first controlled lifecycle apply path, and first local team-scope directory with review-queue coverage cards are now in place. The next large work should continue converting diagnostics into cautious operator workflows, especially richer owner/team review flows, fixture-backed host-path suites, or deeper session/watch evidence automation. The new cleanup memories should remain on quality watch until normal CMU work-cycle receipts prove usefulness or drag.
...
- Practice/Anchor review UX: compact approval moments for high-consequence memory promotion, narrowing, challenge, split, retirement, or strengthening.
...
- Import/export and portability layer: first portable bundle boundary implemented through `cmu portable-export` and `cmu portable-import`; first compatibility gate implemented through `cmu portable-validate`; still needs compatibility tests across future schema versions later.
