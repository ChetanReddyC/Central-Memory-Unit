...
The first-pass structural skeleton is complete, and the scenario-library, quickstart-demo, Python SDK, portable-compatibility, MCP hardening, readiness workflow, document-curation gate, memory seeding workbench, first real workspace cleanup pass, setup-guide adoption surface, README/editable-install path, install-check validation gate, scripted local demo walkthrough, isolated built-distribution validation, first portable compatibility fixture gate, first autonomous-runner hooks, first runner-scenario evidence surface, first Codex host adapter, first before/after scenario comparison, first conservative checkpoint monitor, first compact review queue, first lightweight review reminder surface, first controlled lifecycle apply path, and first local team-scope directory with review-queue coverage cards are now in place. The next large work should continue converting diagnostics into cautious operator workflows, especially richer owner/team review flows, fixture-backed host-path suites, or deeper session/watch evidence automation. The new cleanup memories should remain on quality watch until normal CMU work-cycle receipts prove usefulness or drag.
...
12. Agent integration boundary (first direct tool-call service implemented through `cmu.agent_api.AgentIntegration`, `cmu agent-tools`, and `cmu agent-call`; first SDK, MCP, setup, packaging, autonomous-runner hook surfaces, and Codex runner adapter implemented; expand later with additional host adapters).
13. Team and authority model (first policy/report/apply slice implemented through `cmu authority` and `cmu authority-set`; first local team-scope directory implemented through `cmu team-scope-add` and `cmu team-scope`).
...
- Scenario/evaluation harness: first read-only CLI slice implemented through `cmu evaluate-scenario`, persisted scenario-library runs now exist through `cmu scenario-add`, `cmu scenario-list`, and `cmu scenario-run`, runner-hook lifecycle proof now exists through `cmu runner-scenario`, and before/after behavior comparison now exists through `cmu scenario-compare`; still needs richer fixture repositories, longitudinal evidence cases, and host-path scenario suites later.
...
- Practice/Anchor review UX: compact approval moments and lightweight reminders now exist; still needs richer non-CLI approval, narrowing, challenge, split, retirement, or strengthening surfaces.
- Team/org authority model: first policy/report/apply slice and first local team-scope directory implemented; still needs richer delegation, owner/team review, and cross-repo authority later.
...
- Import/export and portability layer: first portable bundle boundary implemented through `cmu portable-export` and `cmu portable-import`; first bundle validation gate implemented through `cmu portable-validate`; first fixture compatibility gate implemented through `cmu portable-compat`; still needs broader historical/future migration fixture corpus later.
- Product/UI surface: eventual human-facing views for memory paths, review cards, graph/tree navigation, and trust/evidence inspection.
...
- No host-specific autonomous agent runner adapter yet; the versioned direct service, JSON adapter, Python SDK facade, local MCP stdio server, and event-shaped runner hook facade are now the first integration boundaries.
- No automatic Git watcher yet; commit linking and use review remain command-driven through `cmu use-link-auto`, `cmu use-link-latest`, `cmu use-link`, and `cmu use-review`.
...
- Direct autonomous agent runner hooks now exist through `cmu.runner_hooks.AutonomousRunnerHooks`; runtimes can integrate through those hooks, `cmu.agent_api.AgentIntegration`, the JSON adapter, the Python SDK facade, or the MCP stdio adapter instead of parsing `cmu start`. Host-specific runner adapters are still future work.
- No UI or rich team directory/delegation layer yet; the first authority policy and controlled assignment surface now exists.
- Scenario/evaluation harness now has direct read-only evaluation, a persisted scenario library, and isolated runner-hook lifecycle proof. It can validate trigger/onboarding/preflight/action/candidate expectations across saved cases and runner hook start/Candidate/checkpoint outcomes, but it does not yet create fixture repositories, compare before/after agent behavior, or automatically evaluate real time/token/risk reduction.
- Practice/Anchor governance is currently diagnostic only. It exposes authority, evidence, challenge pressure, and allowed paths, but it does not yet approve, narrow, split, retire, or strengthen anything by itself.
