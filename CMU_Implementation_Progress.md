...
- Scenario/evaluation harness: first read-only CLI slice implemented through `cmu evaluate-scenario`, persisted scenario-library runs now exist through `cmu scenario-add`, `cmu scenario-list`, and `cmu scenario-run`, runner-hook lifecycle proof now exists through `cmu runner-scenario`, before/after behavior comparison now exists through `cmu scenario-compare`, and the first generated fixture repository now exists through `cmu fixture-repo-create`; still needs richer fixture catalog, longitudinal evidence cases, and host-path scenario suites later.
