...
class ReviewUiReport:
    ...
    def render(self) -> str:
        ...
        lines.append("Proof Meaning: review cards now have a local HTML surface backed by structured inbox data and exact follow-up commands.")
        return "\n".join(lines)


def build_review_ui(root: Path | str, inbox: ReviewInboxReport, *, write: bool = False) -> ReviewUiReport:
    root_path = Path(root)
    html_text = render_review_html(inbox)
...
def render_review_html(inbox: ReviewInboxReport) -> str:
    cards = "\n".join(render_card(item) for item in inbox.items) or '<p class="empty">No review cards.</p>'
...
<body>
  <header>
    <h1>CMU Review UI</h1>
    <p class="summary">Schema {REVIEW_UI_VERSION}; inbox {REVIEW_INBOX_VERSION}; cards {len(inbox.items)}; urgent {inbox.urgent}; read-only.</p>
  </header>
  <main>
    <section class="grid">
{cards}
